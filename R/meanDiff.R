...
#' @param powerDigits With what precision you want the power to print.
...
#' @param ... Additional arguments are passen on to the [ggplot2::ggplot()]
#' print method.
#'
...
meanDiff <- function(x, y=NULL, paired = FALSE, r.prepost = NULL,
  ...
  if (is.null(y)) {
    ...
    varNames <- c(paste(ufs::extractVarName(x[2]),"(dependent variable)"),
                  paste(ufs::extractVarName(x[3]),"(grouping variable)"));
    ...
    if (is.numeric(x) && (length(unique(stats::na.omit(x))) > 2) &&
        is.numeric(y) && (length(unique(stats::na.omit(y))) > 2)) {
      varNames <- c(ufs::extractVarName(deparse(substitute(x))),
                    ufs::extractVarName(deparse(substitute(y))));
      groupNames <- varNames;
    }     else if (is.numeric(x) && (length(unique(stats::na.omit(y))) == 2)) {
      depVar <- x;
      groupingVar <- y;
      temp <- data.frame(dv = x,
                         group = y);
      temp$group <- as.factor(temp$group);
      varNames <- c(paste(ufs::extractVarName(deparse(substitute(x))), "(dependent variable)"),
                    paste(ufs::extractVarName(deparse(substitute(y))), "(grouping variable)"));
      x <- temp[['dv']][temp[['group']] == levels(temp[['group']])[1]];
      y <- temp[['dv']][temp[['group']] == levels(temp[['group']])[2]];
      groupNames <- c(as.character(levels(temp[['group']])[1]),
                      as.character(levels(temp[['group']])[2]));
    } else if (is.numeric(y) && (length(unique(stats::na.omit(x))) == 2)) {
      depVar <- y;
      groupingVar <- x;
      temp <- data.frame(dv = y,
                         group = x);
      temp$group <- as.factor(temp$group);
      varNames <- c(paste(ufs::extractVarName(deparse(substitute(y))), "(dependent variable)"),
                    paste(ufs::extractVarName(deparse(substitute(x))), "(grouping variable)"));
      x <- temp[['dv']][temp[['group']] == levels(temp[['group']])[1]];
      y <- temp[['dv']][temp[['group']] == levels(temp[['group']])[2]];
      groupNames <- c(as.character(levels(temp[['group']])[1]),
                      as.character(levels(temp[['group']])[2]));
  ...
  if (paired) {
    ...
    res$sd   <- c(sqrt(stats::var(x)), sqrt(stats::var(y)));

    ### Variance of difference
    res$variance <- stats::var(x - y);

    ### Paired samples t-test
    res$objects$t_test <- stats::t.test(x, y, paired = TRUE);
    ...
    res$power <- pwr::pwr.t.test(d=res$meanDiff.d, n=res$n,
                            sig.level=1-conf.level,
                            type="paired",alternative="two.sided");
    ### Also for small, medium, large effects
    res$power.small <- pwr::pwr.t.test(d=.2, n=res$n, sig.level=1-conf.level,
                                  type="paired",alternative="two.sided");
    res$power.medium <- pwr::pwr.t.test(d=.5, n=res$n, sig.level=1-conf.level,
                                   type="paired",alternative="two.sided");
    res$power.large <- pwr::pwr.t.test(d=.8, n=res$n, sig.level=1-conf.level,
                                  type="paired",alternative="two.sided");
    ...
    if (var.equal == "no") {
      ...
      ### Welch's t-test
      res$objects$t_test <- stats::t.test(x, y, var.equal = FALSE);
      ### Use variance from largest sample
      if (res$n[1]> res$n[2]) {
        res$variance <- stats::var(x);
      }
      else {
        res$variance <- stats::var(y);
      }
    }
    else if (var.equal == "yes") {
      res$type = paste0(res$type, "equal variances)");
      ### Student t-test
      res$objects$t_test <- stats::t.test(x, y, var.equal = TRUE);
      ### Compute pooled variance
      x.ss <- stats::var(x) * (res$n[1] - 1);
      y.ss <- stats::var(y) * (res$n[2] - 1);
    ...
    ### Compute power estimates
    res$power <- pwr::pwr.t.test(d=res$meanDiff.d, n=mean(res$n),
                            sig.level=1-conf.level,
                            type="two.sample",alternative="two.sided");
    ### Also for small, medium, large effects
    res$power.small <- pwr::pwr.t.test(d=.2, n=mean(res$n), sig.level=1-conf.level,
                                  type="two.sample",alternative="two.sided");
    res$power.medium <- pwr::pwr.t.test(d=.5, n=mean(res$n), sig.level=1-conf.level,
                                   type="two.sample",alternative="two.sided");
    res$power.large <- pwr::pwr.t.test(d=.8, n=mean(res$n), sig.level=1-conf.level,
                                  type="two.sample",alternative="two.sided");
  ...
  res$ci.usedZ <- abs(stats::qnorm((1-conf.level)/2));
